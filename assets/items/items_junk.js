Game.ItemRepository.define(loc.ItemRockName, {
    name: loc.ItemRockName,
    description: loc.ItemRockDesc,
    habitat: [loc.MapBiomeHabitatAny],
    character: '*',
    foreground: 'white'
});

Game.ItemRepository.define(loc.ItemGauzeName, {
    name: loc.ItemGauzeName,
    description: loc.ItemGauzeDesc,
    habitat: [loc.MapBiomeHabitatAny],
    character: '*',
    foreground: 'white',
});

Game.ItemRepository.define(loc.ItemPlantExtractsName, {
    name: loc.ItemPlantExtractsName,
    description: loc.ItemPlantExtractsDesc,
    habitat: [loc.MapBiomeHabitatNone],
    character: '*',
    foreground: 'white',
});

Game.ItemRepository.define(loc.ItemIntestinesName, {
    name: loc.ItemIntestinesName,
    description: loc.ItemIntestinesDesc,
    habitat: [loc.MapBiomeHabitatAny],
    character: '*',
    foreground: 'white',
});

Game.ItemRepository.define(loc.ItemWoodScrapName, {
    name: loc.ItemWoodScrapName,
    description: loc.ItemWoodScrapDesc,
    habitat: [loc.MapBiomeHabitatAny],
    character: '*',
    foreground: 'white',
});

Game.ItemRepository.define(loc.ItemPillowFeathersName, {
    name: loc.ItemPillowFeathersName,
    description: loc.ItemPillowFeathersDesc,
    habitat: [loc.MapBiomeHabitatNone],
    character: '*',
    foreground: 'white',
});

Game.ItemRepository.define(loc.ItemBottleName, {
    name: loc.ItemBottleName,
    description: loc.ItemBottleDesc,
    habitat: [loc.MapBiomeHabitatAny],
    character: '*',
    foreground: 'white'
});

Game.ItemRepository.define(loc.ItemMTSName, {
    name: loc.ItemMTSName,
    description: loc.ItemMTSDesc,
    habitat: [loc.MapBiomeHabitatNone],
    character: '*',
    foreground: 'white'
});

Game.ItemRepository.define(loc.ItemFuseName, {
    name: loc.ItemFuseName,
    description: loc.ItemFuseDesc,
    habitat: [loc.MapBiomeHabitatAny],
    character: '*',
    foreground: 'white'
});

Game.ItemRepository.define(loc.ItemMagnetName, {
    name: loc.ItemMagnetName,
    description: loc.ItemMagnetDesc,
    habitat: [loc.MapBiomeHabitatAny],
    character: '*',
    foreground: 'white'
});

Game.ItemRepository.define(loc.ItemMetalScrapName, {
    name: loc.ItemMetalScrapName,
    description: loc.ItemMetalScrapDesc,
    habitat: [loc.MapBiomeHabitatAny],
    character: '*',
    foreground: 'white'
});

Game.ItemRepository.define(loc.ItemTapeName, {
    name: loc.ItemTapeName,
    description: loc.ItemTapeDesc,
    habitat: [loc.MapBiomeHabitatAny],
    character: '*',
    foreground: 'white'
});